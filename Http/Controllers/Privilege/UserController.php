...
class UserController extends Controller
{
    ...
    {
        $query = User::select('user.id', 'user.name',  'user.phone')->where('user.is_disabled', '0');

        if (isset($_GET['phone'])) {
            $searchText = urldecode($_GET['phone']);
            $query->where('user.phone', 'like', '%'.$searchText.'%');
        }
...
// 			print_r(int $session);
        ...
        $user->subscription;
        $user->session;
        $user->is_expired_subscription = (int) $user->isSubscriptionExpired($user->subscription);
